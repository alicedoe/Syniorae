...
/**
 * Modèle pour le fichier config.json
...
data class ConfigurationJsonModel(
    ...
    companion object {
        fun createDefault() = ConfigurationJsonModel(
            calendrier_id = "",
            nb_semaines_max = 4,
...
data class IconsJsonModel(
    ...
    fun findMatchingIcon(eventTitle: String): String? {
        ...
        return associations.firstOrNull { association ->
            association.matches(titleLower)
        }?.icone
    }

    fun hasAssociations(): Boolean = associations.isNotEmpty()
}

/**
 * Modèle pour une association d'icône
 */
data class IconAssociationJsonModel(
    ...
    fun matches(eventTitle: String): Boolean {
        return mots_cles.any { keyword ->
            eventTitle.contains(keyword.lowercase())
        }
    }

    fun getKeywordsDisplay(): String {
        return mots_cles.joinToString(", ")
    }
